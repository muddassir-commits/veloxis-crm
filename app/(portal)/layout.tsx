...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!client) {
    return (
      <div className="theme-light bg-bg-light text-text-primary min-h-screen flex items-center justify-center p-6">
        <div className="bg-bg-light border border-border rounded-[10px] p-8 max-w-md w-full shadow-sm text-center space-y-4">
          ...
          <a
            ...
            className="inline-block bg-primary hover:bg-primary/90 text-white text-xs font-bold px-4 py-2 rounded-[7px] transition-colors"
          >
            Sign Out
          </a>
        </div>
    ...
    <div className="theme-light bg-background text-foreground min-h-screen flex flex-col antialiased">
      ...
      <footer className="py-6 border-t border-border bg-card text-center text-[10px] text-muted-foreground font-medium shrink-0">
        ...
        <p className="mt-1 text-[9px] opacity-70">
          Powered by <a href="https://veloxisglobal.com" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline font-bold">Veloxis Global</a>
        </p>
      </footer>
