    ...
    <PageContainer title="Reports & Analytics" description="Compile, review, generate and distribute monthly client performance reports.">
        ...
        <div className="flex flex-wrap gap-3 items-center w-full md:w-auto">
          ...
          <div className="flex flex-col gap-1 min-w-[200px]">
            ...
            <select
              ...
              className="bg-bg-card border border-border/30 text-text-primary rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-blue-500 w-full"
        ...
        {selectedClientId && (
          <div className="flex gap-2 w-full md:w-auto justify-end">
            <Button
              ...
              className={`text-xs ${activeTab === 'dashboard' ? 'bg-border text-text-primary hover:bg-border/80' : 'text-text-secondary'}`}
            >
            ...
            <Button
              ...
              className={`text-xs ${activeTab === 'preview' ? 'bg-border text-text-primary hover:bg-border/80' : 'text-text-secondary'}`}
            >
      ...
      {isPending ? (
        ...
        <div className="rounded-xl border border-border/30 bg-bg-card p-16 text-center text-text-secondary flex flex-col items-center justify-center gap-4 shadow-md">
          <div className="p-4 bg-border/40 rounded-full text-blue-400">
        ...
        /* Portal Preview View */
        <div className="bg-bg-darker border border-border/30 rounded-xl p-4 md:p-8 shadow-inner overflow-hidden">
          <div className="flex justify-between items-center mb-6 border-b border-border/30 pb-4">
            ...
            <Button
              ...
              size="sm"
              onClick={() => setActiveTab('dashboard')}
              className="text-xs border-border/30 text-text-secondary hover:bg-border/40"
            >
              Back to Dashboard
            </Button>
          </div>
              ...
              <CardContent className="pt-5 space-y-4">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  <Button
                    ...
                    className="bg-primary hover:bg-primary/90 text-white text-xs px-4 py-2 font-medium"
                  >
                    {generating ? (
                      <>
                  ...
                  {currentReport && (
                    <>
                      <Button
                        ...
                        className="bg-success hover:bg-success/90 text-white text-xs px-4 py-2 font-medium"
                      >
                      ...
                      {currentReport.files?.public_url && (
                        <a 
                          ...
                          className="inline-flex shrink-0 items-center justify-center rounded-lg border border-border/30 bg-transparent text-sm font-medium whitespace-nowrap transition-all outline-none select-none text-text-secondary hover:bg-border/40 px-2.5 h-8 text-xs"
                        >
                  ...
                  <Button
                    ...
                    className="border-border/30 text-text-secondary hover:bg-border/40 text-xs"
                  >
            ...
            <Card className="bg-bg-card border-border/30 text-text-primary">
              ...
              <CardContent className="pt-5 space-y-4">
                <div className="space-y-3.5">
                  <div className="flex justify-between items-center bg-bg-card border border-border/30/40 p-2.5 rounded-lg">
                    <span className="text-xs font-semibold text-text-primary">Google Search Console</span>
                    <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${isGscConnected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-border/40 text-gray-400 border border-border/30/60'}`}>
                  ...
                  <div className="flex justify-between items-center bg-bg-card border border-border/30/40 p-2.5 rounded-lg">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${isGa4Connected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-border/40 text-gray-400 border border-border/30/60'}`}>
                  ...
                  <div className="flex justify-between items-center bg-bg-card border border-border/30/40 p-2.5 rounded-lg">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${isMetaConnected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-border/40 text-gray-400 border border-border/30/60'}`}>
                  ...
                  <div className="flex justify-between items-center bg-bg-card border border-border/30/40 p-2.5 rounded-lg">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded font-bold ${isGoogleAdsConnected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-border/40 text-gray-400 border border-border/30/60'}`}>
              ...
              <CardContent className="pt-4 space-y-4">
                {seoData ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-2 bg-bg-card border border-border/30/40 p-3 rounded-lg text-center">
                      <div>
                    ...
                    {keywordsData.length > 0 ? (
                      <div className="max-h-[160px] overflow-y-auto border border-border/30/50 rounded-lg">
                        <table className="w-full text-xs text-text-secondary text-left">
                          <thead className="bg-bg-card text-text-primary uppercase text-[10px] border-b border-border/30/50 sticky top-0">
                            <tr>
                          ...
                          <tbody>
                            {keywordsData.slice(0, 5).map((kw, i) => (
                              <tr key={i} className="border-b border-border/30/30 hover:bg-border/20">
                      ...
                      <div className="text-center text-xs text-gray-500 py-6 bg-bg-card rounded-lg border border-dashed border-border/30">
            ...
            <Card className="bg-bg-card border-border/30 text-text-primary">
              ...
              <CardContent className="pt-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  {/* Meta Ads Box */}
                  <div className="bg-bg-card border border-border/30/40 p-4 rounded-lg">
            ...
            <CardContent className="pt-4">
              {initialReports.filter(r => r.client_id === selectedClientId).length > 0 ? (
                <div className="overflow-x-auto border border-border/30/50 rounded-lg">
                  <table className="w-full text-xs text-text-secondary text-left">
                    <thead className="bg-bg-card text-text-primary uppercase text-[10px] border-b border-border/30/50">
                      <tr>
                            ...
                            <div className="flex gap-2 justify-end">
                              ...
                              {r.files?.public_url && (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="inline-flex items-center justify-center rounded-lg border border-transparent text-text-secondary hover:text-text-primary hover:bg-border/40 h-7 w-7 p-0 transition-colors"
                                  title="Download PDF"
                                >
                ...
                <div className="text-center text-xs text-gray-500 py-10 bg-bg-card/40 border border-dashed border-border/30 rounded-lg">
